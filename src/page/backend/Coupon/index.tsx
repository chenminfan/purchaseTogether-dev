...
import { BackendCouponType } from '@typeTS/backend/BCoupon'
import { PaginationType } from '@typeTS/PaginationType'

export default function BackendCoupon() {
  const [couponData, setCouponData] = useState<BackendCouponType[]>([]);
  ...
  const [tamp, setTamp] = useState<BackendCouponType>(state.dataTamp);
  ...
  const SORT_DATA: BackendCouponType[] = useMemo(() => {
    return [...couponData]
    ...
    <>
      <CTableFrom
        ...
        <>
          ...
          <Pagination page={page} getPagination={getCoupon} />
        </>
      </CTableFrom>
      {tableType === 'delete' && (
        <DialogDeleteCoupon
