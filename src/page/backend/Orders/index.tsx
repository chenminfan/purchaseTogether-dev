...
import { BackendOrdersType } from '@typeTS/backend/BOrders'
import { PaginationType } from '@typeTS/PaginationType'


export default function BackendOrders() {
  const [ordersData, setOrdersData] = useState<BackendOrdersType[]>([]);
  ...
  const [tamp, setTamp] = useState<BackendOrdersType>(state.dataTamp);
    ...
    <>
      <CTableFrom
        ...
        <>
          ...
          <Pagination page={page} getPagination={getOrders} />
        </>
      </CTableFrom>
      {tableType === 'delete' && (
        <DialogDeleteOrders
