        ...
        <>
          <TableContainer sx={{ height: 'calc(100% - 182px)', minWidth: '100%' }}>
            <Table stickyHeader aria-label="sticky table">
              ...
              <TableBody>
                {SEARCH_DATA.map((row) => {
                  const Filter_IMAGE = row.imagesUrl?.filter((item) => item.length > 0 && item.includes('https://'))
                  return (
                    <TableRow key={row.id}>
                      <TableCell padding="checkbox">
                        <Checkbox
                      ...
                      <TableCell sx={[
                        ...
                        <div className="row-image">
                          { }
                          {Filter_IMAGE.map((item) => (
                            <div className='img_box' key={`img${row.id}_${item}`}><img src={item} alt={row.id} /></div>))}
                        </div>
